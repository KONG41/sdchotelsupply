...
export default function EditModalBtn({id}:{id:number}) {
    ...
    const {data} = trpc.clientRoute.get.useQuery({id})
    const utils = trpc.useUtils()
    const mutationImg = trpc.clientRoute.updateImgOnDelete.useMutation({
      onError: (err) => {
    ...
    const mutation = trpc.clientRoute.update.useMutation({
      onSuccess: () => {
    ...
    const handleCancel = () => {
      utils.clientRoute.get.invalidate({id})
      if(data){
        setName(data.name)
        setYear(data.year??"")
        setDescription(data.description??"")
