...
export default function EditModalBtn({id}:{id:number}) {
    ...
    const {data} = trpc.career.get.useQuery({id})
    const utils = trpc.useUtils()
    const mutationImg = trpc.career.updateImgOnDelete.useMutation({
      onError: (err) => {
    ...
    const mutation = trpc.career.update.useMutation({
      onSuccess: () => {
    ...
    const handleCancel = () => {
      utils.career.get.invalidate({id})
      if(data){
        setPosition(data.position)
        setTerm(data.term??"")
        setOpenDate(data.openDate??"")
